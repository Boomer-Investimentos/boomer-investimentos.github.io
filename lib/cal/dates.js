...
function diaRecebimentoValido(frequencia, diaRecebimento, mes) {
  if (String(frequencia || '').trim().toLowerCase() === 'semanal') {
    return ocorrenciasDeDiaDaSemana(mes, diaRecebimento).length > 0 ? null : 'Dia da semana inválido.';
  }
  const dia = Number(diaRecebimento);
  if (!Number.isInteger(dia) || dia < 1 || dia > diasNoMes(mes)) {
    return 'Dia inválido para o mês informado.';
  }
  return null;
}

function frequenciaCanonica(valor) {
  const chave = semAcento(String(valor || '')).toLowerCase().trim();
  if (chave === 'mensal') return 'Mensal';
  if (chave === 'semanal') return 'Semanal';
  if (chave === 'unica') return 'Única';
  return null;
}
